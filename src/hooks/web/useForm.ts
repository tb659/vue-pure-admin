...
export const useForm = (props?: PureFormProps) => {
  /** From实例 */
  ...
  async function getForm() {
    ...
    !form && console.error("registered the form first please！");
    return form;
  }
  ...
  const methods: {
    ...
    setProps: async (props: PureFormProps = {}) => {
      const form = await getForm();
      form?.setProps(props);
    },

    setValue: async (data: Recordable) => {
      const form = await getForm();
      form?.setValue(data);
    },
  ...
  return { register, elFormRef, methods };
};
