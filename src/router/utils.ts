...
import { MENU_TYPE, PAGE_BUTTON_PERMISSION_KEY, USER_MENU_LIST_KEY } from "@/utils/constants";
import { getUser, useUserStoreHook } from "@/store/modules/user";
...
function filterShowLinkTree(data: RouteComponent[]) {
  ...
  newTree.forEach((v: { children }) => v.children && (v.children = filterShowLinkTree(v.children)));
  return newTree;
}
...
function filterUserAuthMenu(data: RouteComponent[]) {
  const userAuthMenu = getUser(USER_MENU_LIST_KEY) || [];
  const newTree = cloneDeep(data).filter((v: { menuId: number; meta }) => userAuthMenu.filter(m => v.menuId === m.id).length > 0);
  newTree.forEach(v => v.children?.length && (v.children = filterUserAuthMenu(v.children)));
  return newTree;
}

/** 过滤按钮级别的菜单 fillterBtn = true 过滤按钮  = false 过滤非按钮 */
function filterButtenTree(data: RouteComponent[], fillterBtn: boolean) {
  const newTree = cloneDeep(data).filter((v: { type: number }) =>
    fillterBtn ? v.type !== MENU_TYPE.B_V : v.type === MENU_TYPE.B_V,
  );
  ...
  list.forEach(menu => {
    ...
    try {
      ...
      menuItem.meta = {
        ...
        hiddenTag: meta.hiddenTag ?? false,
        fixedTag: meta.fixedTag ?? false,
...
function handleAsyncRoutes(routeList) {
  if (routeList.length === 0) {
    ...
    // 过滤按钮
    routeList = filterButtenTree(routeList, true);
    // 过滤禁用菜单
...
/** 单点登录 */
function singleSignLogin(oauthCode: string) {
  const { login } = useUserStoreHook();
  return login(null, null, oauthCode);
}
