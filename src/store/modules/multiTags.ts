...
// 延迟创建 store，等待配置加载完成
let multiTagsStore: ReturnType<typeof defineStore>;

export const useMultiTagsStore = () => {
  if (!multiTagsStore) {
    multiTagsStore = defineStore(`${responsiveStorageNameSpace()}store_multiTags`, {
      state: () => ({
        // 存储标签页信息（路由信息）
        multiTags: useSettingStoreHook().getConfigure.multiTagsCache
          ? useSettingStoreHook().getTags
      ...
      persist: {
        pick: ["multiTags", "multiTagsCache"],
      },
    });
  }
  return multiTagsStore(store) as unknown as MultitagsStore;
};
