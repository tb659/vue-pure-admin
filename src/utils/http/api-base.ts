...
import { TOKEN_KEY, SINGLE_CAPTCHA } from "@/utils/constants";
import { usePermissionStoreHook } from "@/store/modules/permission";
import { getToken, setToken, getCookie, getSingleCaptcha } from "@/utils/cookie";

const { resultCode, baseURL, timeout, mockRequest } = httpConfig;

// 相关配置请参考：www.axios-js.com/zh-cn/docs/#axios-request-config-1
const defaultConfig: AxiosRequestConfig = {
  // api 的 baseUrl
  baseURL: mockRequest ? "" : baseURL,
  // 请求超时时间
  timeout,
  headers: {
  ...
  private static retryOriginalRequest(config: PureHttpRequestConfig) {
    return new Promise(resolve => {
      PureHttp.requests.push(() => {
        config.headers[SINGLE_CAPTCHA] = getCookie(`${responsiveStorageNameSpace()}${SINGLE_CAPTCHA}`);
        resolve(config);
      });
    });
  }
  ...
  private httpInterceptorsRequest(): void {
    PureHttp.axiosInstance.interceptors.request.use(
      async (config: PureHttpRequestConfig): Promise<any> => {
        if (import.meta.env.VITE_API_BASEPATH === "pro" && location.hostname === "47.96.19.146") {
          if (
            config.method !== "get" &&
            !(
              ...
              config.url.includes("/users/loginByCode")
            )
          ) {
              ...
              if (data) {
                const leftTime = parseInt(
                  +getCookie(`${responsiveStorageNameSpace()}loginExpireMinutes`) -
                    (Date.now() - +getCookie(`${responsiveStorageNameSpace()}loginTime`)) / 1000 / 60,
                );
      ...
      async (response: PureHttpResponse) => {
        ...
        if (data.code !== resultCode) {
          if (data.code === "err.session" || data.code === "authorization.unauthorized") {
            ...
            location.reload();
          } else if (data.message === "code.used") {
            console.log(usePermissionStoreHook().wholeMenus);
            const route = usePermissionStoreHook().wholeMenus[0];
