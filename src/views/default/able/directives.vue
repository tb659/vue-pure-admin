...
function onInput() {
  msg.info(search.value);
}
function onInputTwo() {
  msg.info(searchTwo.value);
}
function onInputThree({ name, sex }) {
  msg.info(`${name}${sex}${searchThree.value}`);
}

function onInputFour() {
  msg.info(searchFour.value);
}
function onInputFive({ name, sex }) {
  msg.info(`${name}${sex}${searchFive.value}`);
}
...
<template>
  <el-card shadow="never">
    ...
    <div class="mb-2">
      ...
      <el-input v-model="searchTwo" v-optimize="{ event: 'input', fn: onInputTwo, timeout: 400 }" class="!w-[200px]" clearable />
    </div>
    <div>
