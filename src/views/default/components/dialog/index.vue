...
function onOpenClick() {
  addDialog({
    title: "打开后的回调",
    open: ({ options, index }) => msg.info({ options, index } as any),
...
function onFormOneClick() {
  addDialog({
    ...
    closeCallBack: ({ options, args }) => {
      ...
      if (args?.command === "cancel") {
        // 您点击了取消按钮
        msg.info(`您点击了取消按钮，当前表单数据为 ${text}`);
      } else if (args?.command === "sure") {
        msg.info(`您点击了确定按钮，当前表单数据为 ${text}`);
      } else {
        msg.info(`您点击了右上角关闭按钮或者空白页，当前表单数据为 ${text}`);
      }
    }
  });
}
...
function onFormTwoClick() {
  addDialog({
    ...
    closeCallBack: () => {
      msg.info(`当前表单数据为 姓名：${formInline.value.user} 城市：${formInline.value.region}`);
      // 重置表单数据
...
function onFormThreeClick() {
  addDialog({
    ...
    closeCallBack: () => {
      msg.info(`当前表单数据为 姓名：${formThreeInline.value.user} 城市：${formThreeInline.value.region}`);
      // 重置表单数据
...
function onFormFourClick() {
  addDialog({
    ...
    closeCallBack: () => {
      msg.info(`当前表单数据为 姓名：${formFourInline.value.user} 城市：${formFourInline.value.region}`);
      // 重置表单数据
...
function onFormPrimitiveFormClick() {
  addDialog({
    ...
    closeCallBack: () => {
      msg.info(`当前表单内容：${formPrimitiveParam.value}`);
      // 重置表单数据
