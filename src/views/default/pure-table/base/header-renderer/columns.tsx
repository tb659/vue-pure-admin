...
export function useColumns() {
  ...
  const handleEdit = (index: number, row) => {
    msg.success(`您编辑了第 ${index} 行，数据为：${JSON.stringify(row)}`);
  };

  const handleDelete = (index: number, row) => {
    msg.info(`您删除了第 ${index} 行，数据为：${JSON.stringify(row)}`);
  };
  ...
  const columns: TableColumn[] = [
    ...
    },
    {
      align: "right",
      // 自定义表头
      headerRenderer: () => <el-input v-model={search.value} size="small" clearable placeholder="Type to search" />,
