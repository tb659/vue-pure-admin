import { clone, useDark, useECharts, type EchartOptions } from "@pureadmin/utils";
...
export function useColumns() {
  ...
  dataList.value.forEach((_, i) => {
    const { setOptions } = useECharts(templateRef(`PieChartRef${i}`) as Ref<HTMLDivElement>, {
      theme
    });
    ...
    setOptions(
      ...
      },
      {
        ...
        callback: ({ data: { name, value } }) => {
          msg.success(`您点击了第 ${i + 1} 行，图表标题为${name}，图表数据为：${value}`);
        }
      }
    );
  });
