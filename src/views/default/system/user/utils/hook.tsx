...
export function useUser(tableRef: Ref, treeRef: Ref) {
  ...
  function handleDelete(row) {
    msg.success(`您删除了用户编号为${row.id}的这条数据`);
    onSearch();
  }
  ...
  function onbatchDel() {
    ...
    msg.success(`已删除用户编号为 ${getKeyList(curSelected, "id")} 的数据`);
    tableRef.value.getTableRef().clearSelection();
  }
  ...
  function openDialog(title = "新增", row?: FormItemProps) {
    addDialog({
      ...
      beforeSure: (done, { options }) => {
        ...
        function chores() {
          msg.success(`您${title}了用户名称为${curData.username}的这条数据`);
          done(); // 关闭弹框
          onSearch(); // 刷新表格数据
        }
    ...
    addDialog({
      ...
      beforeSure: done => {
        ruleFormRef.value.validate(valid => {
          if (valid) {
            // 表单规则校验通过
            msg.success(`已成功重置 ${row.username} 用户的密码`);
